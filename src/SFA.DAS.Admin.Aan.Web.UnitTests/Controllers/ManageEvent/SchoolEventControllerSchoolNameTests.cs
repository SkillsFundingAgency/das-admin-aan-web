...
public class SchoolEventControllerSchoolNameTests
{
    ...
    {
        var sessionServiceMock = new Mock<ISessionService>();
        var validatorMock = new Mock<IValidator<EventSchoolNameViewModel>>();

        var sessionModel = new EventSessionModel();

        sessionServiceMock.Setup(s => s.Get<EventSessionModel>()).Returns(sessionModel);

        var submitModel = new EventSchoolNameViewModel { Urn = urn };

        var validationResult = new ValidationResult();
        validatorMock.Setup(v => v.Validate(submitModel)).Returns(validationResult);

        var sut = new SchoolEventController(sessionServiceMock.Object, Mock.Of<IValidator<EventAtSchoolViewModel>>(), validatorMock.Object);
        ...
        [Greedy] SchoolEventController sut)
    {
        ...
        var submitModel = new EventSchoolNameViewModel();
